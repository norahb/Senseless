...
# Just calibrate thresholds on validation data
    # def calibrate_thresholds_on_validation(self, X_val, y_val):
    #     """
    #     Calibrate thresholds using validation data for better generalization.
    #     This replaces adjust_method_thresholds() to fix the distribution mismatch.
    #     """
    #     # print("🎯 Calibrating thresholds on validation data...")
    #     print("🎯 Calibrating thresholds on validation data...")
    #     print(f"📊 Validation data: {len(X_val)} samples, {np.sum(y_val)} anomalies ({np.mean(y_val)*100:.1f}%)")
    ...
    #     # Calculate percentiles on validation data
    #     p75 = np.percentile(val_errors, 75)
    #     p85 = np.percentile(val_errors, 85) 
    #     p90 = np.percentile(val_errors, 90)
    #     p95 = np.percentile(val_errors, 95)
        
    #     # # Get config values (with defaults)
    #     # threshold_percentile = getattr(self, 'threshold_percentile', 90)
    #     # threshold_factor = getattr(self, 'threshold_factor', 2.0)

    #     # CORRECT (fix):
    #     threshold_percentile = self.threshold_percentile  # Use actual config values
    #     threshold_factor = self.threshold_factor          # Use actual config values
        
    #     # Calculate validation-based threshold
    #     base_threshold = np.percentile(val_errors, threshold_percentile)
    #     self.global_threshold = base_threshold * threshold_factor

        
    #     print(f"📊 Validation reconstruction error analysis:")
    #     print(f"   75th percentile: {p75:.6f}")
    #     print(f"   85th percentile: {p85:.6f}")
    #     print(f"   90th percentile: {p90:.6f}")
    #     print(f"   95th percentile: {p95:.6f}")
    #     print(f"🎯 New global threshold: {self.global_threshold:.6f} (P{threshold_percentile} * {threshold_factor})")
        
    #     # Check detection rate on validation
    #     val_detections = np.sum(val_errors > self.global_threshold)
    #     detection_rate = val_detections / len(val_errors)
    #     actual_anomaly_rate = np.sum(y_val) / len(y_val)
    ...
    #         iso_detection_rate = np.sum(val_iso_pred == -1) / len(val_iso_pred)
            
    #         # print(f"🌲 Isolation Forest validation rate: {iso_detection_rate:.1%}")
            
    #         # If too different from expected, retrain with better contamination
    #         if abs(iso_detection_rate - actual_anomaly_rate) > 0.3:  # 30% difference
    #             new_contamination = min(0.5, max(0.01, actual_anomaly_rate))
    #             current_contamination = getattr(self, 'contamination', 0.05)  # Default fallback
    #             # print(f"🔧 Retraining Isolation Forest: {current_contamination:.3f} → {new_contamination:.3f}")
    ...
    #             # Verify new rate
    #             new_val_iso_pred = self.isolation_forest.predict(X_val_scaled)
    #             new_iso_rate = np.sum(new_val_iso_pred == -1) / len(new_val_iso_pred)
    #             # print(f"✅ New Isolation Forest rate: {new_iso_rate:.1%}")
        
    #     # === ELLIPTIC ENVELOPE CALIBRATION ===
    ...
    #         outlier_detection_rate = np.sum(val_outlier_pred == -1) / len(val_outlier_pred)
            
    #         print(f"🥚 EllipticEnvelope validation rate: {outlier_detection_rate:.1%}")
            
    #         # If detecting 0% or way too much, retrain
    #         if outlier_detection_rate < 0.01 or outlier_detection_rate > 0.8:
    #             new_contamination = min(0.5, max(0.01, actual_anomaly_rate))
    #             # print(f"🔧 Retraining EllipticEnvelope with contamination: {new_contamination:.3f}")
                
    #             from sklearn.covariance import EllipticEnvelope
    #             self.outlier_detector = EllipticEnvelope(contamination=new_contamination, random_state=42)
    #             self.outlier_detector.fit(X_val_scaled)
                
    #             # Verify new rate
    #             new_val_outlier_pred = self.outlier_detector.predict(X_val_scaled)
    #             new_outlier_rate = np.sum(new_val_outlier_pred == -1) / len(new_val_outlier_pred)
    #             # print(f"✅ New EllipticEnvelope rate: {new_outlier_rate:.1%}")
        
    #     print("✅ Threshold calibration completed!")

# Dynamic optimization for threshold calibration using ROC and F1-score, balace precision-recall
# The dynamic method will automatically choose the best performing approach for your specific data
#  which will likely be the dynamic percentile search method
    # def calibrate_thresholds_on_validation(self, X_val, y_val):
    #     """
    #     Calibrate thresholds using validation data with dynamic optimization.
    #     This replaces adjust_method_thresholds() to fix the distribution mismatch.
    #     """
    #     print("🎯 Calibrating thresholds with dynamic optimization...")
    #     print(f"📊 Validation data: {len(X_val)} samples, {np.sum(y_val)} anomalies ({np.mean(y_val)*100:.1f}%)")

    #     X_val_scaled = self.scaler.transform(X_val)
        
    #     # === AUTOENCODER THRESHOLD CALIBRATION WITH DYNAMIC OPTIMIZATION ===
    #     val_reconstructions = self.autoencoder.predict(X_val_scaled)
    #     val_errors = np.mean((X_val_scaled - val_reconstructions) ** 2, axis=1)
    ...
    #     print(f"   75th percentile: {np.percentile(val_errors, 75):.6f}")
    #     print(f"   90th percentile: {np.percentile(val_errors, 90):.6f}")
    #     print(f"   95th percentile: {np.percentile(val_errors, 95):.6f}")
        
    #     # === DYNAMIC THRESHOLD OPTIMIZATION ===
        
    #     # Method 1: Precision-Recall Curve (Approach 3 - Balanced)
    #     try:
    ...
    #     except:
    #         balanced_threshold = np.percentile(val_errors, 90)
        
    #     # Method 2: ROC-Based Optimal (Approach 4 - Youden's J)
    #     try:
    #         fpr, tpr, roc_thresholds = roc_curve(y_val, val_errors)
    #         # Maximize TPR - FPR (Youden's J statistic)
    #         optimal_idx = np.argmax(tpr - fpr)
    #         roc_threshold = roc_thresholds[optimal_idx]
    #     except:
    #         roc_threshold = np.percentile(val_errors, 85)
        
    #     # Method 3: F1-Score Optimization
    #     try:
    #         threshold_range = np.linspace(np.min(val_errors), np.max(val_errors), 100)
    #         f1_scores = []
    #         for thresh in threshold_range:
    #             preds = (val_errors > thresh).astype(int)
    #             if np.sum(preds) == 0 or np.sum(preds) == len(preds):
    #                 f1_scores.append(0)  # Degenerate case
    #             else:
    #                 f1 = f1_score(y_val, preds, zero_division=0)
    #                 f1_scores.append(f1)
            
    #         best_f1_idx = np.argmax(f1_scores)
    #         f1_threshold = threshold_range[best_f1_idx]
    #     except:
    #         f1_threshold = np.percentile(val_errors, 80)
        
    #     # Method 4: Dynamic Percentile Selection (Most Comprehensive)
    #     try:
    #         best_percentile = self.threshold_percentile
    #         best_factor = self.threshold_factor
    #         best_score = 0
            
    #         # Search through percentile and factor combinations
    #         percentiles = np.arange(60, 98, 2)  # 60, 62, 64, ..., 96
    #         factors = np.arange(0.7, 1.6, 0.1)  # 0.7, 0.8, 0.9, ..., 1.5
            
    #         for p in percentiles:
    #             for factor in factors:
    #                 candidate_threshold = np.percentile(val_errors, p) * factor
    #                 preds = (val_errors > candidate_threshold).astype(int)
    ...
    #                 if np.sum(y_val == 1) > 0 and np.sum(y_val == 0) > 0:
    #                     sensitivity = np.mean(preds[y_val == 1])  # True positive rate
    #                     specificity = np.mean(1 - preds[y_val == 0])  # True negative rate
    ...
    #                 # Penalty for very high or very low detection rates
    #                 detection_rate = np.mean(preds)
    #                 if detection_rate < 0.01 or detection_rate > 0.8:
    #                     score *= 0.5  # Penalize extreme detection rates
    ...
    #         dynamic_threshold = np.percentile(val_errors, best_percentile) * best_factor
    #         dynamic_info = f"P{best_percentile} * {best_factor:.1f}"
            
    #     except Exception as e:
    #         print(f"⚠️ Dynamic optimization failed: {e}")
    #         dynamic_threshold = np.percentile(val_errors, self.threshold_percentile) * self.threshold_factor
    #         dynamic_info = f"P{self.threshold_percentile} * {self.threshold_factor} (fallback)"
        
    #     # Method 5: Original Config-Based
    #     config_threshold = np.percentile(val_errors, self.threshold_percentile) * self.threshold_factor
        
    #     # === EVALUATE ALL THRESHOLD CANDIDATES ===
    #     candidate_thresholds = {
    #         'balanced_pr': balanced_threshold,
    #         'roc_optimal': roc_threshold,
    #         'f1_optimal': f1_threshold,
    #         'dynamic_percentile': dynamic_threshold,
    #         'config_based': config_threshold
    #     }
        
    #     threshold_results = {}
    #     print(f"\n🔍 Evaluating threshold candidates:")
    ...
    #             if np.sum(y_val == 1) > 0 and np.sum(y_val == 0) > 0:
    #                 sensitivity = np.mean(preds[y_val == 1])
    #                 specificity = np.mean(1 - preds[y_val == 0])
    #                 balanced_acc = (sensitivity + specificity) / 2
    #             else:
    #                 balanced_acc = 0
                
    #             # Combined score
    #             combined_score = (f1 + balanced_acc) / 2
    #             detection_rate = np.mean(preds)
                
    #             # Apply penalties for extreme detection rates
    #             if detection_rate < 0.01 or detection_rate > 0.8:
    #                 combined_score *= 0.5
    ...
    #             # Print results
    #             print(f"   {name:20s}: {thresh:.6f} | F1={f1:.3f} | Bal={balanced_acc:.3f} | Score={combined_score:.3f} | Det={detection_rate:.1%}")
    ...
    #     valid_results = {k: v for k, v in threshold_results.items() if v['valid']}
        
    #     if valid_results:
    #         best_method = max(valid_results.keys(), 
    #                         key=lambda k: valid_results[k]['combined_score'])
    #         self.global_threshold = valid_results[best_method]['threshold']
    #         best_score = valid_results[best_method]['combined_score']
            
    #         print(f"\n🎯 Selected method: {best_method}")
    #         print(f"🎯 Optimal threshold: {self.global_threshold:.6f}")
    #         print(f"🎯 Combined score: {best_score:.3f}")
            
    #         if best_method == 'dynamic_percentile':
    #             print(f"🎯 Dynamic config: {dynamic_info}")
                
    #     else:
    #         # Fallback to config-based
    #         self.global_threshold = config_threshold
    #         print(f"⚠️ All methods failed, using config fallback: {self.global_threshold:.6f}")
        
    #     # === VALIDATE FINAL THRESHOLD ===
    #     final_preds = (val_errors > self.global_threshold).astype(int)
    #     final_detection_rate = np.mean(final_preds)
    #     actual_anomaly_rate = np.mean(y_val)
        
    #     print(f"\n📈 Final threshold validation:")
    #     print(f"   Detection rate: {final_detection_rate:.1%}")
    #     print(f"   Actual anomaly rate: {actual_anomaly_rate:.1%}")
    #     print(f"   Ratio: {final_detection_rate/actual_anomaly_rate:.2f}x")
    ...
    #         # If too different from expected, retrain with better contamination
    #         if abs(iso_detection_rate - actual_anomaly_rate) > 0.3:
    #             new_contamination = min(0.5, max(0.01, actual_anomaly_rate))
    #             current_contamination = getattr(self, 'contamination', 0.05)
    ...
    #     print("✅ Dynamic threshold calibration completed!")
        
    #     # Store optimization results for later reference
    #     self.threshold_optimization_results = threshold_results
    #     self.selected_threshold_method = best_method if valid_results else 'config_fallback'
        
    #     return self

    # Memory-optimized calibration for large validation sets
    # def calibrate_thresholds_on_validation(self, X_val, y_val):
    #     """
    #     Memory-optimized calibration using sampling for large validation sets.
    #     """
    #     print("🎯 Calibrating thresholds on validation data...")
        
    #     # MEMORY FIX: Sample validation data if too large
    #     if len(X_val) > 10000:  # Threshold for sampling
    #         print(f"⚠️ Large validation set ({len(X_val)} samples), sampling for memory efficiency...")
    ...
    #     # Check detection rate on validation sample
    #     val_detections = np.sum(val_errors > self.global_threshold)
    #     detection_rate = val_detections / len(val_errors)
    #     actual_anomaly_rate = np.sum(y_val_sample) / len(y_val_sample)
        
    #     print(f"📈 Validation calibration results:")
    #     print(f"   Detection rate: {detection_rate:.1%}")
    #     print(f"   Actual anomaly rate: {actual_anomaly_rate:.1%}")
        
    #     # === ISOLATION FOREST CALIBRATION (with memory check) ===
    #     if hasattr(self, 'isolation_forest'):
    #         try:
    #             val_iso_pred = self.isolation_forest.predict(X_val_scaled)
    #             iso_detection_rate = np.sum(val_iso_pred == -1) / len(val_iso_pred)
    ...
    #                 self.contamination = new_contamination
    #         except MemoryError:
    #             print("⚠️ Memory error in Isolation Forest calibration, skipping...")
        
    #     # === ELLIPTIC ENVELOPE CALIBRATION (with memory check) ===
    #     if hasattr(self, 'outlier_detector') and self.outlier_detector is not None:
    #         try:
    #             val_outlier_pred = self.outlier_detector.predict(X_val_scaled)
    #             outlier_detection_rate = np.sum(val_outlier_pred == -1) / len(val_outlier_pred)
                
    #             # If detecting poorly, retrain
    #             if outlier_detection_rate < 0.01 or outlier_detection_rate > 0.8:
    #                 new_contamination = min(0.5, max(0.01, actual_anomaly_rate))
    #                 from sklearn.covariance import EllipticEnvelope
    #                 self.outlier_detector = EllipticEnvelope(contamination=new_contamination, random_state=42)
    #                 # Use smaller sample for training if needed
    #                 train_sample_size = min(2000, len(X_val_scaled))
    #                 if len(X_val_scaled) > train_sample_size:
    #                     train_indices = np.random.choice(len(X_val_scaled), train_sample_size, replace=False)
    #                     self.outlier_detector.fit(X_val_scaled[train_indices])
    #                 else:
    #                     self.outlier_detector.fit(X_val_scaled)
    #         except MemoryError:
    #             print("⚠️ Memory error in EllipticEnvelope calibration, disabling...")
...
# # Deployment-ready wrapper class
# class EnvironmentAdaptiveDetector:
#     """
#     High-level wrapper for easy deployment to new environments.
#     """
    
#     def __init__(self, environment_name, sensor_names):
#         """
#         Initialize detector for a new environment.
        
#         Parameters:
#         -----------
#         environment_name : str
#             Name of the environment (e.g., "factory_floor_1", "office_building_a")
#         sensor_names : list
#             List of sensor names in the environment
#         """
#         self.environment_name = environment_name
#         self.detector = AdaptiveUnsupervisedAutoencoder(
#             sensor_names=sensor_names,
#             case_name=environment_name
#         )
#         self.deployment_log = []
        
#     def deploy_to_environment(self, initial_data, min_days=3):
#         """
#         Deploy detector to a new environment with initial learning period.
        
#         Parameters:
#         -----------
#         initial_data : numpy.ndarray or pandas.DataFrame
#             Initial data from the environment (assumed normal operation)
#         min_days : int
#             Minimum days of data recommended for stable deployment
            
#         Returns:
#         --------
#         deployment_status : dict
#             Status of deployment including recommendations
#         """
#         print(f"🚀 Deploying to environment: {self.environment_name}")
        
#         # Convert to numpy array if DataFrame
#         if isinstance(initial_data, pd.DataFrame):
#             X = initial_data[self.detector.sensor_names].values
#         else:
#             X = initial_data
            
#         # Check data quality
#         data_quality = self._assess_data_quality(X)
        
#         if data_quality['sufficient_data']:
#             # Train the detector
#             self.detector.learn_environment(X)
...
#                 'data_quality': data_quality,
#                 'recommendations': self._get_deployment_recommendations(data_quality),
#                 'ready_for_production': data_quality['production_ready']
#             }
            
#             print(f"✅ Successfully deployed to {self.environment_name}")
#             return status
...
#     def _assess_data_quality(self, X):
#         """
#         Assess the quality of training data for deployment.
#         """
#         n_samples, n_features = X.shape
        
#         # Basic checks
#         sufficient_samples = n_samples >= self.detector.min_training_samples
#         no_missing_values = not np.isnan(X).any()
#         no_constant_features = np.std(X, axis=0).min() > 1e-8
        
#         # Temporal coverage (assuming hourly data)
#         hours_of_data = n_samples  # Adjust based on your sampling rate
#         days_of_data = hours_of_data / 24
#         sufficient_duration = days_of_data >= 1  # At least 1 day
        
#         # Variability check
#         cv_values = np.std(X, axis=0) / (np.abs(np.mean(X, axis=0)) + 1e-8)
#         sufficient_variability = np.mean(cv_values) > 0.01
        
#         # Production readiness
#         production_ready = (sufficient_samples and 
#                           sufficient_duration and 
#                           no_missing_values and 
#                           no_constant_features and 
#                           sufficient_variability)
        
#         return {
#             'sufficient_data': sufficient_samples,
#             'samples': n_samples,
#             'days_coverage': days_of_data,
#             'sufficient_duration': sufficient_duration,
#             'no_missing_values': no_missing_values,
#             'no_constant_features': no_constant_features,
#             'sufficient_variability': sufficient_variability,
#             'production_ready': production_ready,
#             'quality_score': np.mean([
#                 sufficient_samples, sufficient_duration, 
#                 no_missing_values, no_constant_features, sufficient_variability
#             ])
#         }
    
#     def _get_deployment_recommendations(self, data_quality):
#         """
#         Get recommendations based on data quality assessment.
#         """
#         recommendations = []
        
#         if not data_quality['sufficient_data']:
#             recommendations.append(f"Collect at least {self.detector.min_training_samples} samples")
        
#         if not data_quality['sufficient_duration']:
#             recommendations.append("Collect data for at least 1-3 days of normal operation")
        
#         if not data_quality['no_missing_values']:
#             recommendations.append("Handle missing values in sensor data")
        
#         if not data_quality['no_constant_features']:
#             recommendations.append("Check for non-functional sensors (constant values)")
        
#         if not data_quality['sufficient_variability']:
#             recommendations.append("Ensure sensors show normal operational variability")
        
#         if data_quality['production_ready']:
#             recommendations.append("✅ Ready for production deployment")
#         else:
#             recommendations.append("⚠️ Additional data collection recommended before production")
            
#         return recommendations
    
#     def monitor_realtime(self, new_data_batch, use_home_optimization=True, sensitivity='medium'):        
#         """
#         # Parameters:
#         -----------
#         new_data_batch : numpy.ndarray or pandas.DataFrame
#             New batch of sensor data
...
#             return {'error': 'Detector not deployed. Call deploy_to_environment() first.'}
        
#         # Convert to numpy array if DataFrame
#         if isinstance(new_data_batch, pd.DataFrame):
#             X = new_data_batch[self.detector.sensor_names].values
#         else:
#             X = new_data_batch
            
#         # Predict anomalies
#         anomaly_flags, anomaly_scores, drift_info = self.detector.predict_anomalies(X, adapt=True)
...
#             'sensor_scores': anomaly_scores,
#             'drift_info': drift_info,
#             'status': 'normal' if np.mean(anomaly_flags) < 0.1 else 'attention_needed',
#             'alerts': self._generate_alerts(anomaly_flags, drift_info)
#         }
...
#     def _generate_alerts(self, anomaly_flags, drift_info):
#         """
#         Generate actionable alerts based on monitoring results.
#         """
#         alerts = []
...
#                 'action': 'Investigate sensor readings and environment conditions'
#             })
#         elif anomaly_rate > 0.1:
#             alerts.append({
#                 'level': 'MEDIUM',
#                 'message': f'Elevated anomaly rate: {anomaly_rate:.1%}',
#                 'action': 'Monitor closely for trending issues'
#             })
        
#         if drift_score > 0.5:
#             alerts.append({
#                 'level': 'HIGH',
#                 'message': f'Significant environment drift detected: {drift_score:.2f}',
#                 'action': 'Consider retraining model or investigating environment changes'
#             })
#         elif drift_score > 0.3:
#             alerts.append({
#                 'level': 'MEDIUM',
#                 'message': f'Environment drift detected: {drift_score:.2f}',
#                 'action': 'Model adaptation in progress'
#             })
        
#         if drift_info['needs_adaptation']:
#             alerts.append({
#                 'level': 'INFO',
#                 'message': 'Model is adapting to environment changes',
#                 'action': 'Automatic adaptation in progress'
#             })
        
#         return alerts
    
#     def _generate_home_alerts(self, anomaly_flags, insights, drift_info):
#         """
#         Generate home-specific alerts based on insights.
        
#         Parameters:
#         -----------
#         anomaly_flags : numpy.ndarray
#             Binary anomaly flags
#         insights : dict
#             Home insights with anomaly explanations
#         drift_info : dict
#             Environment drift information
...
#         # Process home insights for alerts
#         for i, explanation in enumerate(insights['anomaly_explanations']):
#             if explanation.get('type') == 'climate_issue':
#                 alerts.append({
#                     'level': 'MEDIUM',
#                     'message': 'Climate control issue detected',
#                     'details': explanation['message'],
#                     'action': explanation['recommendation'],
#                     'timestamp_index': i,
#                     'sensors_affected': explanation.get('affected_sensors', [])
#                 })
#             elif explanation.get('type') == 'air_quality':
#                 alerts.append({
#                     'level': 'HIGH',
#                     'message': 'Air quality concern',
#                     'details': explanation['message'],
#                     'action': explanation['recommendation'],
#                     'timestamp_index': i,
#                     'sensors_affected': explanation.get('affected_sensors', [])
#                 })
#             elif explanation.get('type') == 'activity_pattern':
#                 alerts.append({
#                     'level': 'HIGH',
...
#                     'sensors_affected': explanation.get('affected_sensors', [])
#                 })
#             elif explanation.get('type') == 'sensor_specific':
#                 alerts.append({
#                     'level': 'MEDIUM',
...
#                     'sensors_affected': explanation.get('affected_sensors', [])
#                 })
#             elif explanation.get('type') == 'general_anomaly':
#                 alerts.append({
#                     'level': 'LOW',
#                     'message': 'General environmental change',
#                     'details': explanation['message'],
#                     'action': explanation['recommendation'],
#                     'timestamp_index': i
#                 })
        
#         # Add general alerts for high anomaly rates
#         if anomaly_rate > 0.2:
#             alerts.append({
#                 'level': 'HIGH',
#                 'message': f'High anomaly rate: {anomaly_rate:.1%}',
#                 'action': 'Investigate multiple environmental factors',
#                 'details': 'Multiple sensors showing unusual patterns',
#                 'anomaly_rate': anomaly_rate
#             })
#         elif anomaly_rate > 0.1:
#             alerts.append({
...
#                 'message': f'Elevated anomaly rate: {anomaly_rate:.1%}',
#                 'action': 'Monitor closely for trending issues',
#                 'details': 'Some sensors showing unusual patterns',
#                 'anomaly_rate': anomaly_rate
#             })
...
#             alerts.append({
#                 'level': 'HIGH',
#                 'message': 'Significant environment drift detected',
#                 'action': 'Consider retraining model or investigating environment changes',
...
#             alerts.append({
#                 'level': 'INFO',
#                 'message': 'Environment patterns are changing',
#                 'action': 'Model is adapting to new conditions',
#                 'details': f'Drift score: {drift_score:.2f}',
#                 'drift_score': drift_score
#             })
        
#         # Add system status alerts
#         if drift_info.get('needs_adaptation', False):
#             alerts.append({
#                 'level': 'INFO',
#                 'message': 'Model is adapting to environment changes',
#                 'action': 'Automatic adaptation in progress',
#                 'details': 'System learning new patterns'
#             })
        
#         return alerts
    
#     def get_environment_health(self):
#         """
#         Get overall health status of the monitored environment.
#         """
#         if not self.detector.is_trained:
#             return {'status': 'not_deployed'}
        
#         # Get detector status
#         status_report = self.detector.get_status_report()
        
#         # Calculate health metrics
#         recent_logs = [log for log in self.deployment_log 
#                       if log['timestamp'] > datetime.now() - timedelta(hours=24)]
        
#         recent_anomaly_rate = np.mean([log.get('anomalies', 0) for log in recent_logs if log['action'] == 'monitoring'])
#         recent_drift_score = np.mean([log.get('drift_score', 0) for log in recent_logs if log['action'] == 'monitoring'])
        
#         # Health scoring
#         health_score = 1.0
#         if recent_anomaly_rate > 0.1:
#             health_score -= 0.3
#         if recent_drift_score > 0.3:
#             health_score -= 0.2
#         if status_report['drift_status']['needs_attention']:
#             health_score -= 0.2
...
#             'recent_drift_score': recent_drift_score,
#             'uptime_hours': (datetime.now() - self.deployment_log[0]['timestamp']).total_seconds() / 3600 if self.deployment_log else 0,
#             'total_samples_processed': status_report['samples_processed'],
#             'model_status': status_report,
#             'recommendations': self._get_health_recommendations(health_score, recent_anomaly_rate, recent_drift_score)
#         }
    
#     def _get_health_recommendations(self, health_score, anomaly_rate, drift_score):
#         """
...
#         if health_score < 0.4:
#             recommendations.append("🚨 URGENT: Environment health is poor. Immediate investigation required.")
        
#         if anomaly_rate > 0.15:
#             recommendations.append("⚠️ High anomaly rate. Check for equipment malfunctions or environmental changes.")
        
#         if drift_score > 0.4:
#             recommendations.append("🔄 Significant drift detected. Consider manual model retraining.")
        
#         if health_score > 0.8:
#             recommendations.append("✅ Environment operating normally. Continue monitoring.")
        
#         return recommendations
    
#     def save_deployment(self, filepath=None):
#         """
#         Save the entire deployment state.
#         """
#         if filepath is None:
#             filepath = os.path.join(self.detector.base_path, f"deployment_{self.environment_name}")
        
#         # Save detector
#         self.detector.save_adaptive_model(filepath)
...
#             'detector_config': {
#                 'sensor_names': self.detector.sensor_names,
#                 'adaptation_window': self.detector.adaptation_window,
#                 'min_training_samples': self.detector.min_training_samples
#             }
#         }
        
#         with open(f"{filepath}_deployment.json", 'w') as f:
#             json.dump(deployment_data, f, indent=2, default=str)
        
#         print(f"✅ Deployment saved to {filepath}")
#         return self
    
#     def load_deployment(self, filepath=None):
#         """
...
#         # Load detector
#         self.detector.load_adaptive_model(filepath)
        
#         # Load deployment log
#         with open(f"{filepath}_deployment.json", 'r') as f:
#             deployment_data = json.load(f)
            
#         self.environment_name = deployment_data['environment_name']
#         self.deployment_log = deployment_data['deployment_log']
...
#         print(f"✅ Deployment loaded from {filepath}")
#         return self

# # Example usage and quick deployment script
# def quick_deploy_to_environment(environment_name, sensor_names, initial_data_path, 
#                                data_format='csv', sensor_cols=None):
#     """
#     Quick deployment function for new environments.
...
#         Path to initial training data
#     data_format : str
#         Format of data file ('csv', 'json', 'parquet')
#     sensor_cols : list
#         Column names if different from sensor_names
        
#     Returns:
#     --------
#     detector : EnvironmentAdaptiveDetector
#         Deployed detector ready for monitoring
#     """
#     print(f"🚀 Quick deployment to: {environment_name}")
    
#     # Load data
#     if data_format == 'csv':
#         df = pd.read_csv(initial_data_path)
#     elif data_format == 'json':
#         df = pd.read_json(initial_data_path)
...
#     initial_data = df[sensor_cols].values
    
#     # Create and deploy detector
#     detector = EnvironmentAdaptiveDetector(environment_name, sensor_names)
#     deployment_status = detector.deploy_to_environment(initial_data)
    
#     if deployment_status['deployed']:
#         print(f"✅ Successfully deployed to {environment_name}")
#         print(f"📊 Data quality score: {deployment_status['data_quality']['quality_score']:.2f}")
#         print(f"🎯 Production ready: {deployment_status['ready_for_production']}")
        
#         # Save deployment
#         detector.save_deployment()
        
#         return detector
#     else:
#         print(f"❌ Deployment failed: {deployment_status['reason']}")
...
#     # def learn_environment(self, X, retrain=False):
#     #     """
#     #     Learn the normal behavior of the environment from unlabeled data.
#     #     This is the main training function for deployment.
...
#     #     retrain : bool
#     #         Whether this is a retraining on new environment
#     #     """
#     #     print(f"\n🎓 Learning environment from {X.shape[0]} samples...")
        
#     #     if X.shape[0] < self.min_training_samples:
#     #         raise ValueError(f"Need at least {self.min_training_samples} samples, got {X.shape[0]}")
        
#     #     # Auto-configure if needed
#     #     if self.auto_scaling_method:
#     #         self.scaler = self._choose_scaler(X)
#     #     else:
#     #         self.scaler = RobustScaler()
            
#     #     if self.auto_contamination:
#     #         self.current_contamination = self._estimate_contamination(X)
...
#     #     print(f"🏗️  Architecture: {self.n_features} -> {' -> '.join(map(str, hidden_layers))} -> {self.n_features}")
#     #     print(f"⚙️  Solver: {solver}, Max iterations: {max_iter}")
...
#     #     self.isolation_forest = IsolationForest(
#     #         contamination=self.current_contamination,
#     #         random_state=42,
#     #         n_estimators=100
#     #     )
#     #     self.isolation_forest.fit(X_scaled)
...
#     #         print(f"⚠️ EllipticEnvelope failed: {e}")
#     #         print("🔄 Using fallback: IsolationForest as outlier detector")
#     #         self.outlier_detector = None  # Mark as unavailable

#     #     # Calculate baseline statistics and thresholds
#     #     self._calculate_adaptive_thresholds(X_scaled)
        
#     #     # Store baseline statistics for drift detection
#     #     self._store_baseline_stats(X_scaled)
        
#     #     # Initialize buffers
#     #     self.training_data_buffer.extend(X)
#     #     self.recent_samples.extend(X[-self.adaptation_window:])
