...
# def run(config, sensor_model=None, recent_sensor_data=None):
    ...
    if recent_sensor_data is None:
        decision_df = pd.read_csv(config.decision_log_path)
        # Get recent data (last window_size entries)
        recent_sensor_data = decision_df.tail(100)
    ...
    # Detect reconstruction drift (if sensor model provided)
    reconstruction_drift = False
    reconstruction_results = {}
    if sensor_model is not None:
        reconstruction_drift, reconstruction_results = detector.detect_reconstruction_drift(
            sensor_model, recent_sensor_data
        )
    
    # Log results
    detector.log_drift_results(statistical_results, reconstruction_results)
    
    # Overall drift decision
    overall_drift = statistical_drift or reconstruction_drift
    ...
    print(f"📊 Drift Detection Summary:")
    print(f"   Statistical drift: {'Yes' if statistical_drift else 'No'}")
    print(f"   Reconstruction drift: {'Yes' if reconstruction_drift else 'No'}")
    print(f"   Overall drift detected: {'Yes' if overall_drift else 'No'}")
    print(f"   Recommend retraining: {'Yes' if should_retrain else 'No'}")
